...
const validateCreateUser = [
  ...
  // Validation conditionnelle pour le champ 'subjectIds' si le rôle est 'professor'
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .send({ success: false, message: "Cet utilisateur n'existe pas" });
    }
...
const verifyUser = async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .send({ success: false, message: "Aucun token fourni" });
    }
