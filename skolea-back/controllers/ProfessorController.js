...
async function associateSubjectsToProfessor(
  professorId,
  subjects,
  transaction
) {
  ...
  try {
    ...
    res
      ...
      .json({
        success: false,
        message:
          "Une erreur est survenue lors de la récupération des professeurs",
        error: error.message,
      });
  }
}
...
async function createProfessorWithCity(data, transaction) {
  ...
  try {
    ...
    console.error(
      "Erreur lors de la création du professeur avec la ville:",
      error
    );
    throw error;
  }
...
async function searchProfessors(req, res) {
  ...
  if (!subject || !city) {
    return res
      ...
      .json({
        success: false,
        message: "Both subject and city parameters are required.",
      });
  }
